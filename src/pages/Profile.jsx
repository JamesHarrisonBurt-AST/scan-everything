      ...
      {!loading && stats.totalValue > 0 && (
        <div className="px-4 mt-3">
          <motion.div
            ...
            <div>
              <p className="text-[11px] text-muted-foreground uppercase tracking-wider">Estimated Vault Value</p>
              ...
              <Link key={item.id} to={item.identified_item_id ? `/scan-result/${item.identified_item_id}` : '#'}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.best_price_found && (
                    <p className="text-[11px] text-cyan-400">${item.best_price_found.toFixed(2)}</p>
                  )}
                </div>
